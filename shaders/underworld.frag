...
{
    float f = 0.0;
    f += 0.500000*(0.1+0.5*noise( p )); p = m*p*2.02;
    f += 0.250000*(0.5+0.9*noise( p )); p = m*p*2.03;
...
{
    vec3 pos = _position + vec3(0.,0.,iGlobalTime * -15.5);
...
float map(vec3 pos)
{
    ...
    float sminFactor = 10.25;
    dist = smin(dist,sphere(vec3(0.,-30.,80.),35.),sminFactor);
    return dist;
}
